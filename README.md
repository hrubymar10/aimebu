...
POST   /api/usages/settings            Update usage refresh interval (minimum 15s), percent display ("left" or "used"), provider order, Claude auto-refresh (claude_auto_refresh), warmup mode (warmup_mode: off/force/scheduled/smart), and multiline cron schedule (warmup_schedule)
