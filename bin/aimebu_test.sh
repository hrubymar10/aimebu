...
with_test_server_env() {
    AIMEBU_TLS_CERT= \
    AIMEBU_TLS_KEY= \
    AIMEBU_BIND=127.0.0.1 \
    AIMEBU_ALLOW=127.0.0.0/8,::1/128 \
    "$@"
}

cleanup() {
    if [ -n "${TEST_CONFIG_DIR:-}" ]; then
        AIMEBU_CONFIG_DIR="$TEST_CONFIG_DIR" with_test_server_env "$WRAPPER" server stop >/dev/null 2>&1 || true
    fi
...
with_test_server_env "$WRAPPER" server start >/dev/null
...
AIMEBU_CONFIG_DIR="$TEST_CONFIG_DIR" AIMEBU_PORT="$TEST_PORT" with_test_server_env "$WRAPPER" server status | grep -q "aimebu is running"
