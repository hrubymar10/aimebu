...
func pruneCmd(args []string) {
	...
	if !autoYes {
		...
		if all {
			...
			fmt.Println("This will permanently delete runtime agent state only:")
			fmt.Println("  • server/agents.json         (all registered agents — they re-register on reconnect)")
			fmt.Println("  • agents/agent-sessions.json (aimebu agent resume state)")
			fmt.Println("  • agents/agent-logs/*        (always-on stderr + optional JSONL diagnostics)")
			fmt.Println()
			fmt.Println("Preserved (use -a to wipe these too):")
			fmt.Println("  • server/rooms.json, server/messages.json (full conversation history)")
			fmt.Println("  • server/macros.json, server/fleet.json  (macros, fleet bundles)")
			fmt.Println("  • agents/agent-warning-acknowledged (first-run warning acknowledgement)")
			fmt.Println("  • server/aimebu.pid, server/aimebu.log (runtime artifacts)")
		}
		fmt.Print("\nAre you sure? [y/N]: ")
