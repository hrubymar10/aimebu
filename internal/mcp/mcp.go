...
var tools = []tool{
	...
	{
		Name:        "bus_register",
		Description: "Do not send unprompted \"hello\" or \"standing by\" messages after registering. Only speak when the user explicitly asks you to, or in reply to another agent. If the user said \"connect/join/wait\", call bus_wait directly — no greeting first. REQUIRED FIRST CALL for aimebu message-bus work. Register yourself on the messagebus before using any other bus tool, but do not register solely to unlock another bus tool such as bus_recall or bus_memory_list; register only when the user's task is actually about collaborating on the aimebu message bus. The server will assign you a random slug (e.g. 'alice') and assemble your full agent ID like 'alice@aimebu'. For `model`: pass 'unknown' unless your system prompt explicitly states your model version — do not guess or copy examples. For `harness`: pass your harness slug if you know it for certain (e.g. 'claude-code', 'codex', 'cursor', 'pi'); if unsure, omit the field entirely so the server can auto-detect it — do NOT pass 'unknown' for harness. The returned 'id' is your full identity for all subsequent calls. The optional `name` + `force=true` pair force-claims that slug under the current project; `--resume-name` in the wrapper is the prior-identity resume path.",
		InputSchema: inputSchema{
			Type: "object",
			Properties: map[string]property{
				"model":   {Type: "string", Description: "Short version slug only if your system prompt explicitly states the model version; otherwise pass 'unknown'. Do not infer or copy example values."},
				"harness": {Type: "string", Description: "Your harness slug (e.g. 'claude-code', 'codex', 'cursor', 'pi'). Pass if known for certain. If uncertain, omit this field entirely — do NOT pass 'unknown', as that suppresses auto-detection which is load-bearing for some harnesses (e.g. codex)."},
