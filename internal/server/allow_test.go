...
func TestValidateBindAddrDefault(t *testing.T) {
	t.Setenv("AIMEBU_BIND", "")
	t.Setenv("AIMEBU_PORT", "")
	if err := validateBindAddr(DefaultAddr()); err != nil {
		t.Fatalf("default resolution should not error: %v", err)
	}
}

func TestValidateBindAddrOverrideHonoured(t *testing.T) {
	for _, val := range []string{"0.0.0.0:9997", "172.28.47.1:9997", "[::1]:9997", "[::]:9997"} {
		t.Run(val, func(t *testing.T) {
			if err := validateBindAddr(val); err != nil {
				t.Errorf("%q should be accepted: %v", val, err)
			}
		})
	}
}

func TestValidateBindAddrRejectsInvalid(t *testing.T) {
	cases := []struct{ name, val string }{
		{"empty host", ":9997"},
		{"hostname", "localhost:9997"},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if err := validateBindAddr(tc.val); err == nil {
...
func TestResolveAllowMixedIPAndCIDR(t *testing.T) {
	t.Setenv("AIMEBU_ALLOW", "127.0.0.1, 172.28.47.0/24,172.28.47.62")
	prefixes, err := resolveAllow()
	if err != nil {
...
func TestResolveAllowDedupes(t *testing.T) {
	t.Setenv("AIMEBU_ALLOW", "127.0.0.1, 127.0.0.1/32, 172.28.47.0/24, 172.28.47.0/24")
	prefixes, err := resolveAllow()
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestResolveAllowAcceptsIPv6(t *testing.T) {
	t.Setenv("AIMEBU_ALLOW", "::1, fe80::/10")
	prefixes, err := resolveAllow()
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
