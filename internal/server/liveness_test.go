...
func TestSweepAgentLivenessLadder(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "offline alert", age: 6 * time.Minute, want: types.AgentStateOffline, wantEvts: 1},
	} {
...
func TestSweepAgentLivenessOfflineEdgeFiresOnceAndReconnectRearms(t *testing.T) {
	...
	now := time.Now().UTC()
	setAgentLastSeen(t, s, agent.ID, now.Add(-6*time.Minute))
	...
	setAgentLastSeen(t, s, agent.ID, now.Add(-7*time.Minute))
...
func TestSweepAgentLivenessOpenWaitAndWebsocketKeepActive(t *testing.T) {
	...
	now := time.Now().UTC()
	setAgentLastSeen(t, s, waiter.ID, now.Add(-6*time.Minute))
	setAgentLastSeen(t, s, wsAgent.ID, now.Add(-6*time.Minute))
...
func TestEmitLivenessOfflineTargetsHumansAndNamesRole(t *testing.T) {
	...
	now := time.Now().UTC()
	setAgentLastSeen(t, s, ai.ID, now.Add(-6*time.Minute))
